...
try {
  ...
  __webpack_require__(/*! ./modules/fog */ "./assets/src/js/modules/fog.js");
  //  require("./modules/input_mask");
  __webpack_require__(/*! ./modules/generall */ "./assets/src/js/modules/generall.js");
  __webpack_require__(/*! ./modules/tabs */ "./assets/src/js/modules/tabs.js");
...
if (canvas !== null) {
  ...
  var FogParticle = /*#__PURE__*/function () {
    function FogParticle(x, y, size, speed) {
...
var canvasMob = document.getElementById('fogCanvasMobile');
if (canvasMob !== null) {
  // Инициализируем частицы
  var _initFog = function _initFog() {
    var fogCount = 17; // Количество частиц
    for (var i = 0; i < fogCount; i++) {
      var size = Math.random() * 300 + 128;
      var x = Math.random() * canvas.width;
      var y = Math.random() * (canvas.height - size); // Ограничиваем y при создании
      var speed = Math.random() * 2 + 0.5;
      _fogParticles.push(new _FogParticle(x, y, size, speed));
    }
  }; // Анимация
  var _animateFog2 = function animateFog() {
    _ctx.clearRect(0, 0, canvas.width, canvas.height);
    _fogParticles.forEach(function (particle) {
      particle.update();
      particle.draw();
    });
    requestAnimationFrame(_animateFog2);
  }; // Обновляем размеры холста при изменении окна
  var _ctx = canvasMob.getContext('2d');
  canvasMob.width = window.innerWidth;
  canvasMob.height = window.innerHeight;
  var _fogParticles = [];
  var _fogImage = new Image();
  _fogImage.src = '/wp-content/uploads/2024/12/smoke7-1.png'; // Замените на свою текстуру тумана

  // Создаем частицы тумана
  var _FogParticle = /*#__PURE__*/function () {
    function _FogParticle(x, y, size, speed) {
      _classCallCheck(this, _FogParticle);
      this.x = x;
      this.y = y;
      this.size = size;
      this.speed = speed;
    ...
    return _createClass(_FogParticle, [{
      key: "draw",
      value: function draw() {
        _ctx.globalAlpha = this.opacity;
        _ctx.drawImage(_fogImage, this.x, this.y, this.size, this.size);
        _ctx.globalAlpha = 1.0; // Возвращаем прозрачность
      }
    }, {
      key: "update",
      value: function update() {
  ...
  window.addEventListener('resize', function () {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    _fogParticles.length = 0;
    _initFog();
  });

  // Запуск
  _fogImage.onload = function () {
    _initFog();
    _animateFog2();
  };
}
...
/***/ "./assets/src/js/modules/generall.js":
/*!*******************************************!*\
  !*** ./assets/src/js/modules/generall.js ***!
  \*******************************************/
/***/ (() => {
...
var itemTab = document.querySelectorAll(".tab"),
...
itemTab.forEach(function (item) {
  item.addEventListener("click", function () {
    itemTab.forEach(function (tabs) {
      tabs.classList.remove("active");
    });
    item.classList.add("active");
    var getAttrControls = item.getAttribute("aria-controls");
    panelList.forEach(function (panelItem) {
      var itemID = panelItem.getAttribute("id");
      var setHeightPanel = panelItem.clientHeight;
      if (getAttrControls === itemID) {
        panelBox.style.height = "".concat(setHeightPanel, "px");
      }
      getAttrControls === itemID ? (panelItem.classList.remove("invisible", "opacity-0", "scale-90"), panelItem.classList.add("visible", "opacity-100", "scale-100")) : (panelItem.classList.add("invisible", "opacity-0", "scale-90"), panelItem.classList.remove("visible", "opacity-100", "scale-100"));
    });
  });
});
panelList.forEach(function (panelItem) {});
var setHeightPanel = panelList[0].clientHeight;
panelBox.style.height = "".concat(setHeightPanel, "px");
var container = document.querySelector('.tabList');
var isTouching = false;
var startY = 0;
var scrollStart = 0;
var targetScroll = 0; // Место, к которому хотим прокрутить
var currentScroll = 0; // Текущее положение прокрутки
var ease = 0.6; // Степень плавности прокрутки (чем меньше, тем плавнее)
...
container.addEventListener('touchmove', function (e) {
  ...
  var deltaX = touchMove - startX; // Сколько мы провели пальцем по оси X
...
function smoothScroll() {
  if (Math.abs(currentScroll - targetScroll) > 0.5) {
    // Если текущее положение не слишком близко к целевому
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkadvocate_theme"] = self["webpackChunkadvocate_theme"] || [];
