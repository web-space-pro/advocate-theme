...
if (canvasMob !== null) {
  // Инициализируем частицы
  var initFogMob = function initFogMob() {
    var fogCount = 17; // Количество частиц
    for (var i = 0; i < fogCount; i++) {
      var size = Math.random() * 300 + 128;
      var x = Math.random() * canvas.width;
      var y = Math.random() * (canvas.height - size); // Ограничиваем y при создании
      var speed = Math.random() + 0.37;
      fogParticlesMob.push(new FogParticleMob(x, y, size, speed));
    }
  }; // Анимация
  var _animateFogMob = function animateFogMob() {
    _ctx.clearRect(0, 0, canvas.width, canvas.height);
    fogParticlesMob.forEach(function (particle) {
      particle.update();
      particle.draw();
    });
    requestAnimationFrame(_animateFogMob);
  }; // Обновляем размеры холста при изменении окна
  var _ctx = canvasMob.getContext('2d');
  canvasMob.width = window.innerWidth;
  canvasMob.height = window.innerHeight;
  var fogParticlesMob = [];
  var fogImageMob = new Image();
  fogImageMob.src = '/wp-content/uploads/2024/12/smoke7-1.png'; // Замените на свою текстуру тумана

  // Создаем частицы тумана
  var FogParticleMob = /*#__PURE__*/function () {
    function FogParticleMob(x, y, size, speed) {
      _classCallCheck(this, FogParticleMob);
      this.x = x;
      this.y = y;
      this.size = size;
      this.speed = speed;
    ...
    return _createClass(FogParticleMob, [{
      key: "draw",
      value: function draw() {
        _ctx.globalAlpha = this.opacity;
        _ctx.drawImage(fogImageMob, this.x, this.y, this.size, this.size);
